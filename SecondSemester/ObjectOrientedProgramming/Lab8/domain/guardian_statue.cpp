...
GuardianStatue::GuardianStatue(const std::string &power_word_name, const std::string &material, int age,
...
bool GuardianStatue::operator==(const GuardianStatue &other) const {
    return power_word_name_ == other.power_word_name_ && material_ == other.material_ && age_ == other.age_ &&
...
std::string GuardianStatue::to_csv() {
    std::stringstream result;
    result << power_word_name_ << ",";
    result << material_ << ",";
    result << age_ << ",";
...
std::string GuardianStatue::to_html() {
    std::stringstream result;
    result << "<tr>\n";
    result << "<td>" << power_word_name_ << "</td>";
    result << "<td>" << material_ << "</td>";
    result << "<td>" << age_ << "</td>";
    result << "<td>" << corporeal_form_ << "</td>\n";
    result << "</tr>";
    return result.str();
}

std::istream &operator>>(std::istream &input, GuardianStatue &guardian_statue) {
    input >> guardian_statue.power_word_name_ >> guardian_statue.material_ >> guardian_statue.age_ >>
            guardian_statue.corporeal_form_;
    return input;
}

std::ostream &operator<<(std::ostream &output, const GuardianStatue &guardian_statue) {
    output << guardian_statue.power_word_name_ << ' ' << guardian_statue.material_ << ' '
           << std::to_string(guardian_statue.age_) << ' ' << guardian_statue.corporeal_form_;
    return output;
}
