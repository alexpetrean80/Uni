...
void test_save__valid_power_word_name__statue_is_pushed_to_list() {
    ...
    repo.add(*statue);
    repo.save("aaa");
    assert(repo.get_currently_in_animation().at(0) == *statue);
}
...
void tests_repo() {
    ...
    test_next__first_call__first_statue_is_returned();
    test_save__valid_power_word_name__statue_is_pushed_to_list();
    test_save__invalid_power_word_name__exception_thrown();
}
...
void test_next__mode_b__first_item_is_returned() {
    ...
    service.add_statue("aaa", "bbb", 1, "ccc");
    service.set_mode("B");
    GuardianStatue statue{"aaa", "bbb", 1, "ccc"};
    assert(service.next() == statue);
}
void test_next__mode_a__exception_thrown() {
    Service service;
    service.add_statue("aaa", "bbb", 1, "ccc");
    try {
        GuardianStatue statue = service.next();
        assert(false);
    } catch (...) {
        assert(true);
    }
...
void test_save__mode_b__item_is_saved() {
    ...
    service.save("aaa");
    GuardianStatue statue{"aaa", "bbb", 1, "ccc"};
    assert(service.get_currently_in_animation().at(0) == statue);
}
...
void tests_service() {
    ...
    test_add_service_mode_b_exception_thrown();
    test_delete_service_mode_a_statue_deleted();
    test_delete_service_mode_b_exception_thrown();
    test_update_service_mode_a_statue_updated();
    test_update_service_mode_b_exception_thrown();
    test_get_statues_service_valid_statues_returned();
    test_next__mode_b__first_item_is_returned();
    test_next__mode_a__exception_thrown();
    test_save__mode_b__item_is_saved();
    test_save__mode_a__exception_is_thrown();
}
